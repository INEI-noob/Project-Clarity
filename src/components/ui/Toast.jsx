...
export const ToastProvider = ({ children }) => {
  ...
  return (
    <ToastContext.Provider value={{ addToast }}>
      {children}
      <div className="fixed bottom-8 left-1/2 -translate-x-1/2 z-[200] flex flex-col gap-3 w-full max-w-sm px-4">
        <AnimatePresence>
          {toasts.map((toast) => (
            <motion.div
              key={toast.id}
              ...
              className={`
                glass-sanctuary px-6 py-4 rounded-2xl shadow-2xl flex items-center gap-3 border-l-4
